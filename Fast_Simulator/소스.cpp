#include <iostream> 
 ...
         // Overwrite 
         if (Data_set[sec_num] >= -2 && oper == 'w') {
            if(Data_set[sec_num] == -2){ Data_set[sec_num]++; w_cnt++; continue;}
            r_cnt++; w_cnt++;
             int LSN = sec_num; 
 ...
                                 r_cnt += free_block_cnt; 
                                for(int w = 0; w < Sector_Cnt; w++){
                                  if(Data_set[RPBN*Sector_Cnt + w] == -2){
                                    free_block_cnt--;
                                  }
                                }
 ...
             else if (offset != seq && offset == 0) {
               r_cnt += seq_block_cnt;                  
               for(int w = 0; w < Sector_Cnt; w++){
                   if(Data_set[SPBN*Sector_Cnt + w] == -2){
                    seq_block_cnt--; }
                }
                w_cnt += seq_block_cnt;
                 SPBN = LSN / Sector_Cnt;
 ...
                 seq_block_cnt = Sector_Cnt; 
                 // wear_level[LBN*Sector_Cnt + offset]++; 
                 for (int a = 1; a < Sector_Cnt; a++) { swmp[a] = -1; } 
